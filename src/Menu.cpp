...
Menu menuInicializar() {
	FILE *f = fopen("assets/menu/main.mnu", "r");
	char word[255];
	int nButtons, iButton=0;
	Menu m;
	
	// cargando el menu
    pantalla1 = IMG_Load("assets/imagenes/intro/c.png");
    pantalla2 = IMG_Load("assets/imagenes/intro/ginc.png");
    pantalla3 = IMG_Load("assets/imagenes/intro/super.png");
    pantalla4 = IMG_Load("assets/imagenes/intro/crea.bmp");
    pantalla5 = IMG_Load("assets/imagenes/intro/intro.png");
    pantalla6 = IMG_Load("assets/imagenes/intro/exit.png");
	...
	// cargando musica
    cancion = Mix_LoadMUS("assets/sonidos/fondo.ogg");
    cancion_juego = Mix_LoadMUS("assets/sonidos/carrera.ogg");

    motor = Mix_LoadWAV ( "assets/sonidos/run.wav" );
	Mix_VolumeChunk(motor, 100);           

    velbaja = Mix_LoadWAV ( "assets/sonidos/velocida_baja.wav" );
	Mix_VolumeChunk(velbaja, 10);           

    velalta = Mix_LoadWAV ( "assets/sonidos/velocida_alta.wav" );
	Mix_VolumeChunk(velalta, 10);       
	cancion = Mix_LoadMUS("assets/sonidos/carrera.ogg");
    
    est1 = Mix_LoadMUS("assets/sonidos/radio4.ogg");
    est2 = Mix_LoadMUS("assets/sonidos/radio3.ogg");        
    est3 = Mix_LoadMUS("assets/sonidos/radio2.ogg");        
    est4 = Mix_LoadMUS("assets/sonidos/radio1.ogg");        
    est5 = Mix_LoadMUS("assets/sonidos/radio10.ogg");        
    est6 = Mix_LoadMUS("assets/sonidos/radio9.ogg");       
    est7 = Mix_LoadMUS("assets/sonidos/radio6.ogg");                
    est8 = Mix_LoadMUS("assets/sonidos/radio5.ogg");
...
int menu(Mapa* mapa, int nAncho, int nAlto, bool fullscreen) {
	...
	// datos estaticos
	strcpy(mapa->fileName, "assets/maps/carrerav2.ase");
	...
	SDL_Window* window = SDL_CreateWindow("Menu window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, nAncho, nAlto, flags);
	...
	SDL_RenderSetLogicalSize(renderer, nAncho, nAlto);
	...
	while (!done) {
		...
		for (int i = 0; i < m.nBotones; i++) {
			Boton* bt = &m.botones[i];
			if (bt->seleccionado) {
				selected = SDL_CreateRGBSurface(NULL, bt->img->w + 10, bt->img->h + 10, 32, 0, 0, 0, 0);
